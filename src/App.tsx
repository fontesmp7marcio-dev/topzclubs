...
export default function App() {
  ...
  // This prevents 125 simultaneous HTTP requests on page startup that saturated Render container sockets and caused the 1-minute loading delay
  useEffect(() => {
    if (!dateMatches || dateMatches.length === 0) return;

    // Collect IDs of favorite clubs that actually have matches scheduled today
    const activeFavoritesToFetch: { id: number; name: string }[] = [];

    dateMatches.forEach((m) => {
      const fav1 = supabaseFavorites.find(
        (f) => (m.team1Id && f.id === m.team1Id) || normalizeTeamName(f.name) === normalizeTeamName(m.team1)
      );
      if (fav1 && fav1.id && !fetchedFormsRef.current.has(fav1.id)) {
        fetchedFormsRef.current.add(fav1.id);
        activeFavoritesToFetch.push({ id: fav1.id, name: fav1.name });
      }
      const fav2 = supabaseFavorites.find(
        (f) => (m.team2Id && f.id === m.team2Id) || normalizeTeamName(f.name) === normalizeTeamName(m.team2)
      );
      if (fav2 && fav2.id && !fetchedFormsRef.current.has(fav2.id)) {
        fetchedFormsRef.current.add(fav2.id);
        activeFavoritesToFetch.push({ id: fav2.id, name: fav2.name });
      }
    });

    if (activeFavoritesToFetch.length === 0) return;

    // Fetch active teams in gentle batches of 2 with small delay to maintain zero latency
    let isCancelled = false;
    async function loadActiveTeamForms() {
      for (let i = 0; i < activeFavoritesToFetch.length; i += 2) {
        if (isCancelled) break;
        const batch = activeFavoritesToFetch.slice(i, i + 2);
        await Promise.all(
          batch.map(async (club) => {
            try {
              const res = await fetch(`/api/fotmob/team/${club.id}?name=${encodeURIComponent(club.name)}`);
              if (res.ok) {
                const data = await res.json();
                if (!isCancelled && data && Array.isArray(data.pastMatches) && data.pastMatches.length > 0) {
                  setTeamRawMatches((prev) => ({ ...prev, [club.id]: data.pastMatches }));
                }
              }
            } catch {
              fetchedFormsRef.current.delete(club.id);
            }
          })
        );
        // Small pause between batches to yield CPU and network
        if (i + 2 < activeFavoritesToFetch.length) {
