...
export const FotMobHeader: React.FC<FotMobHeaderProps> = ({
  ...
  // Check if we are inside Google AI Studio dev workspace or if explicitly enabled via ?admin=true
  const isStudioOrAdmin = React.useMemo(() => {
    if (typeof window === 'undefined') return true;
    const host = window.location.hostname;
    const search = window.location.search;
    
    // Explicit override via ?admin=true or ?notificacoes=true
    if (search.includes('admin=true') || search.includes('notificacoes=true')) {
      return true;
    }

    // Google AI Studio preview domain or localhost
    if (host.includes('run.app') || host.includes('localhost') || host === '127.0.0.1') {
      return true;
    }

    // Hide in production deployments like Render
    return false;
  }, []);
