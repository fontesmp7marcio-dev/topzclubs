...
export const BalancoView: React.FC<BalancoViewProps> = ({
  ...
  const groupedData: MonthGroup[] = useMemo(() => {
    ...
    for (const b of sorted) {
      ...
      // Profit of this bet (Single Source of Truth)
      const betProfit = calculateBetProfit(b.status, b.amount, b.odd);
    ...
    <div id="balanco-financeiro-container" className="w-full max-w-7xl mx-auto px-3 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
          ...
          >
            ...
            <span className="hidden sm:inline">
              Atual:{' '}
              <strong className={stats.currentBankroll >= initialCapital ? 'text-emerald-400' : 'text-rose-400'}>
                {stats.currentBankroll.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} R$
              </strong>
            </span>
            <Settings className="w-3.5 h-3.5 text-zinc-400" />
          </button>

          {/* Sincronizar Resultados dos Jogos */}
          <button
            id="btn-sync-bets-results"
            onClick={() => handleSyncAllMatchResults(false)}
            disabled={isSyncing}
            className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-[#18181c] hover:bg-[#22222a] border border-[#2a2a32] hover:border-emerald-500/50 text-zinc-300 hover:text-emerald-400 text-xs font-bold transition-all cursor-pointer shadow-xs disabled:opacity-50"
            title="Sincronizar resultados reais de todas as partidas com suas apostas"
          >
      ...
      {activeBalancoTab === 'simulador' ? (
        ...
        <div 
          ...
          className="bg-[#141416] border border-[#222226] p-4 sm:p-5 rounded-2xl relative group hover:border-[#32323a] transition-all flex flex-col justify-between"
        >
          <div>
            <div className="flex items-center justify-between text-zinc-400 mb-1.5">
              ...
              <div className="relative group/tooltip">
                ...
                <div className="absolute right-0 bottom-full mb-2 hidden group-hover/tooltip:block w-56 p-2.5 bg-[#1f1f26] border border-[#33333d] text-[10px] text-zinc-300 rounded-lg shadow-xl z-30 pointer-events-none leading-relaxed">
                  Total de <strong>{stats.totalBets} apostas</strong> registradas no sistema. Os indicadores de Lucro, ROI e Progressão são apurados com base nas <strong>{stats.settledBets} apostas finalizadas</strong>.
                </div>
              </div>
            </div>
          ...
          <div className="pt-2 text-[10px] font-bold text-zinc-400 flex items-center gap-1.5 flex-wrap">
            <span className="text-zinc-300">{stats.settledBets} finalizadas</span>
            <span className="text-zinc-600">•</span>
            <span className="text-emerald-400">{stats.wonBets}V</span>
            <span className="text-zinc-600">/</span>
            <span className="text-rose-400">{stats.lostBets}D</span>
            {stats.pendingBets > 0 && (
              <>
                <span className="text-zinc-600">•</span>
                <span className="text-amber-400">{stats.pendingBets} pend.</span>
              </>
            )}
          </div>
        </div>
        ...
        >
            ...
            <div className="flex items-center justify-between text-zinc-400 mb-1.5">
              ...
              <div className="relative group/tooltip">
                ...
                <div className="absolute right-0 bottom-full mb-2 hidden group-hover/tooltip:block w-52 p-2 bg-[#1f1f26] border border-[#33333d] text-[10px] text-zinc-300 rounded-lg shadow-xl z-30 pointer-events-none">
                  (Total de ganhos − Total apostado) das {stats.settledBets} apostas liquidadas. Pendentes não somam no lucro realizado.
                </div>
              </div>
            </div>
            <div
          ...
          <div className="pt-2 text-[10px] font-bold text-zinc-400">
            Sobre {stats.settledBets} apostas liquidadas
          </div>
        </div>
          ...
          <div>
            <div className="flex items-center justify-between text-zinc-400 mb-1.5">
              ...
              <div className="relative group/tooltip">
                <HelpCircle className="w-3.5 h-3.5 text-zinc-500 hover:text-zinc-300 cursor-help" />
                <div className="absolute right-0 bottom-full mb-2 hidden group-hover/tooltip:block w-52 p-2 bg-[#1f1f26] border border-[#33333d] text-[10px] text-zinc-300 rounded-lg shadow-xl z-30 pointer-events-none">
                  O ROI mede a relação entre os lucros obtidos e o volume apostado: (Lucros / Total apostado) × 100.
                </div>
              </div>
            </div>
            <div
        ...
        >
          <div>
            <div className="flex items-center justify-between text-zinc-400 mb-1.5">
              ...
              <div className="relative group/tooltip">
                <HelpCircle className="w-3.5 h-3.5 text-zinc-500 hover:text-zinc-300 cursor-help" />
                <div className="absolute right-0 bottom-full mb-2 hidden group-hover/tooltip:block w-52 p-2 bg-[#1f1f26] border border-[#33333d] text-[10px] text-zinc-300 rounded-lg shadow-xl z-30 pointer-events-none">
                  A progressão é a evolução sobre o capital inicial: (Lucros / Capital inicial) × 100.
                </div>
              </div>
            </div>
            <div
          ...
          <div className="pt-2 text-[10px] font-bold text-zinc-400">
            Banca atual: {stats.currentBankroll.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} R$
          </div>
        </div>
                          ...
                          {day.items.map((bet) => {
                            ...
                            return (
                              <div
                                ...
                                className="bg-[#131316] hover:bg-[#16161b] border border-[#24242c] hover:border-zinc-700 p-3 sm:p-3.5 rounded-xl transition-all flex flex-col gap-2.5 group relative cursor-pointer select-none"
                                title="Clique para expandir/recolher os times, ou clique duplo para editar"
                              >
                                {/* Top Bar: Action & Meta badges (Left) | Status, Toggle & Financial Stats (Right) */}
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2 border-b border-[#1f1f28]/70">
                                  {/* Action & Meta badges */}
                                  <div className="flex items-center justify-between sm:justify-start w-full sm:w-auto gap-2 shrink-0">
                                    ...
                                    {/* Mobile Expand Indicator Icon (Only visible on mobile at the end of the line) */}
                                    <div className="sm:hidden p-1 text-zinc-500">
                                      {expandedMatches[bet.id] ? (
                                  ...
                                  {/* Right Section: Status Pill + Saldo + Toggle Button + Chevron */}
                                  <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-2.5 w-full sm:w-auto flex-wrap sm:flex-nowrap shrink-0">
                                    {/* Saldo / Lucro Badge - ALWAYS visible so when card is collapsed the balance is crystal clear */}
                                    <span
                                      className={`px-2.5 py-1 rounded-lg text-[10px] sm:text-[10.5px] font-black tracking-tight font-sans border shrink-0 ${
                                        bet.profit > 0
                                    ...
                                    >
                                      {bet.profit > 0 && '+'}
                                      {bet.profit.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} R$
                                    </span>

                                    {/* Static Status Badge */}
                                    <span
                                      className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider border shrink-0 shadow-xs ${
                                        bet.status === 'Ganha'
                                    ...
                                    <button
                                      ...
                                      className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-[#17171c] hover:bg-[#202028] border border-[#262630] hover:border-zinc-700 text-zinc-400 hover:text-white text-[10px] sm:text-[11px] font-black tracking-tight transition-all cursor-pointer select-none active:scale-95 shrink-0"
                                    ...
                                    >
                                      {isStatsExpanded ? (
                                        ...
                                        <>
                                          ...
                                          <span className="font-sans font-bold">Odds</span>
                                          <ChevronLeft className="w-4 h-4 text-zinc-500" />
                                        </>
                                      )}
                                    </button>

                                    {/* Desktop Expand Matches Chevron (Hidden on mobile) */}
                                    <div className="hidden sm:block p-1 text-zinc-500">
                                      {expandedMatches[bet.id] ? (
                                ...
                                {/* Confrontos do Bilhete com Logos e Nomes Por Extenso - COLLAPSIBLE */}
                                {expandedMatches[bet.id] && (
                                  <div className="w-full pt-1 sm:pt-2 animate-in fade-in duration-200">
                                    {bet.legs && bet.legs.length > 0 ? (
                                      bet.legs.length === 1 ? (
                                        // Confronto único (Aposta Simples)
                                        (() => {
                                          const leg = bet.legs[0];
                                          const legStatus = leg.status || 'pending';
                                          const scoreStr = leg.score?.ft ? `${leg.score.ft[0]}-${leg.score.ft[1]}` : null;
                                          const marketText = leg.marketLabel || (leg.market ? getMarketLabel(leg.market) : null);
                                        ...
                                        // Múltiplos confrontos (Até 5 por bilhete - Diagramação limpa em coluna única, super legível no mobile)
                                      ...
                                      (() => {
                                        const parsed = parseMatchupFromTitle(bet.title);
                                        if (parsed) {
                                          return (
                                            <MatchupPill
                                              team1={parsed.team1}
                                        ...
                                        return (
                                          <div className="px-3 py-2 rounded-lg bg-[#16161f] border border-[#262635] text-xs font-semibold text-white">
                                            {bet.title}
                                          </div>
                                        );
                                      })()
