...
function isFemaleMatchInScraper(leagueName?: string, team1?: string, team2?: string): boolean {
  const check = (str?: string): boolean => {
    ...
    // 0. Protect legitimate top-flight clubs with numbers/Roman numerals (e.g. Willem II)
    if (/\bwillem\s+ii\b/i.test(lower)) {
      return false;
    }
    ...
    // 2. Youth / Sub / U-XX categories (including Dutch "Jong")
    if (
      /\b(sub[- ]?\d{1,2}|u[- ]?\d{1,2})\b/i.test(lower) ||
      /\b(youth|juniores|júnior|junior|juvenil|reserves|reserve|reservas|reserva|jong)\b/i.test(lower)
    ) {
      return true;
    }
...
// Resilient HTTP fetch with strict timeout to avoid blocking Render and Node event loop
...
export async function scrapeFotMobAllLeagues(locale = 'pt-BR', forceRefresh = false) {
  ...
  if (!forceRefresh) {
    ...
    if (cached && Date.now() - cached.timestamp < 15 * 60 * 1000) { // 15 min cache for league directory
      return cached.data;
    }
  }
...
export async function searchFotMobTeam(term: string) {
  ...
  try {
    const res = await fetchWithTimeout(url, { headers: HEADERS }, 4500);
    if (res.ok) {
...
function parseFixturesPayload(teamData: any, teamId: number, teamNameHint?: string) {
  ...
  if (Array.isArray(rawFixtures) && rawFixtures.length > 0) {
    for (const f of rawFixtures) {
      ...
      const isTeamHome =
        (homeId && teamId && Number(homeId) === Number(teamId)) ||
        (homeTeam && teamName && homeTeam.toLowerCase().trim() === teamName.toLowerCase().trim()) ||
        (homeTeam && teamNameHint && homeTeam.toLowerCase().trim() === teamNameHint.toLowerCase().trim());
...
export async function scrapeFotMobTeamFixtures(teamId: number, teamSlug = 'team', forceRefresh = false, teamNameHint?: string) {
  const normHint = (teamNameHint || '').toLowerCase().trim();
  const cacheKey = teamId > 0 ? `fotmob:team-fixtures:${teamId}` : `fotmob:team-fixtures:hint:${normHint}`;
  const TEAM_CACHE_TTL = 20 * 60 * 1000; // 20 min cache for fast repeat loads
  if (!forceRefresh) {
    const cached = memoryCache.get(cacheKey);
  ...
  const setInCache = (data: any) => {
    if (!data) return;
    if (teamId > 0) memoryCache.set(`fotmob:team-fixtures:${teamId}`, { data, timestamp: Date.now() });
    if (data.teamId && data.teamId > 0) memoryCache.set(`fotmob:team-fixtures:${data.teamId}`, { data, timestamp: Date.now() });
    if (normHint) memoryCache.set(`fotmob:team-fixtures:hint:${normHint}`, { data, timestamp: Date.now() });
    if (data.teamName) memoryCache.set(`fotmob:team-fixtures:hint:${data.teamName.toLowerCase().trim()}`, { data, timestamp: Date.now() });
  };

  // 1. Ultra-fast direct JSON API path (~200ms vs multi-second HTML scraping)
  if (teamId > 0) {
    try {
      const apiRes = await fetchWithTimeout(`https://www.fotmob.com/api/data/teams?id=${teamId}`, { headers: HEADERS }, 3500);
      if (apiRes.ok) {
        const apiJson = await apiRes.json();
        const parsed = parseFixturesPayload(apiJson, teamId, teamNameHint);
        if (parsed) {
          setInCache(parsed);
          return parsed;
        }
      ...
      // Proceed to candidate HTML URLs
    }
  }

  // 2. Candidate HTML URLs fallback
  if (teamId > 0) {
    const candidateUrls = [
    ...
    for (const url of candidateUrls) {
      try {
        ...
        const parsed = parseFixturesPayload(teamData, teamId, teamNameHint);
        if (parsed) {
          setInCache(parsed);
          return parsed;
        }
  ...
  if (teamNameHint) {
    try {
      ...
      if (searchRes?.team && searchRes.team.id && searchRes.team.id !== teamId) {
        ...
        const recursiveResult = await scrapeFotMobTeamFixtures(searchRes.team.id, 'team', forceRefresh, teamNameHint);
        if (recursiveResult) {
          setInCache(recursiveResult);
...
export async function scrapeFotMobTodayMatches() {
  ...
  try {
    const res = await fetchWithTimeout(url, { headers: HEADERS }, 6000);
    if (!res.ok) return [];
...
export async function scrapeFotMobMatchesByDate(dateStr: string): Promise<any[]> {
  ...
  try {
    const res = await fetchWithTimeout(url, { headers: HEADERS }, 6500);
    if (!res.ok) {
