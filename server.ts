...
import { apwinRouter } from './server/apwinAnalyzer';
import { checkBatchMatchesAvailability } from './server/oddsChecker';
...
// API: APWin Analyzer
app.use('/api/analysis', apwinRouter);

// API: Odds Availability (Bet365 / Betano for Over/Under 1.5)
app.post('/api/odds-availability/batch', async (req, res) => {
  try {
    const { matches } = req.body;
    if (!matches || !Array.isArray(matches)) {
      return res.status(400).json({ success: false, error: 'Matches array is required' });
    }
    const availability = await checkBatchMatchesAvailability(matches);
...
app.post('/api/user-bets', async (req, res) => {
  try {
    ...
    if (success && (bet.status === 'Ganha' || bet.status === 'Perdida')) {
      handleBetResultNotification(bet).catch((e) => console.warn('Error sending bet result push:', e));
    }
    res.json({ success });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
...
app.post('/api/notifications/settings', async (req, res) => {
  try {
    ...
    const settings = await updateNotificationSettings(newSettings);
    res.json({ success: true, settings });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    // Background pre-warming: pre-load today's matches into RAM cache to ensure instant first-load
    try {
      const today = getBrasiliaTodayStr();
      scrapeFotMobMatchesByDate(today).then(() => {
        console.log(`[Cache Pre-warm] Pre-cached matches for ${today} in RAM.`);
      }).catch(() => {});
      fetchSharedFavorites().catch(() => {});
    } catch {
